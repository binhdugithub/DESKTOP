...
public:
	...
	int GetContentLeng(const char*buffer)
	{
		...
		size_t x = szHeader.find("Content-Length:");
		...
		{
			x += strlen("Content-Length:") + 1;
			size_t y = szHeader.find("\r\n", x);
			std::string valueNum = szHeader.substr(x, y);

			int value = atoi(valueNum.c_str());
			//printf("\nContent leng:%d\n", value);
