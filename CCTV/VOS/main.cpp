...
int main(int argc, char *argv[])
{
	...
	if(!Config::readConfig("vosconfig.xml"))
	{
	...
	DBControl::create();
	if(!DBControl::getSingleton().openDB(dbHost.c_str(), dbUser.c_str(), dbPwd.c_str(), dbName.c_str()))
	{
	...
	string multicastIp = Config::multicastIp;
	int multicastPort = Config::multicastPort;
	LOG_INFO("Create multicast processing ip="<<multicastIp<<" port="<<multicastPort);
	MulticastProcessing* mulProc = new MulticastProcessing();
	if(mulProc->initMulticast(multicastIp, multicastPort))
		mulProc->startListenning();
	else
	{
		std::cout<<"cannot create multicast"<<endl;
		LOG_ERROR("Can not create multicast");
		return -1;
	}
