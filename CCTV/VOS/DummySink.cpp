...
//
//  if (fStreamId != NULL)
//	  cout<<"Stream name = "<<fStreamId<<" Media name = "<<fSubsession.mediumName()<<" Codec name = "
//	  << fSubsession.codecName()<<" frame size = "<<frameSize<<endl;
//
//  if (numTruncatedBytes > 0)
//	  cout<<"bytes truncated "<<numTruncatedBytes<<endl;
//
//  //char uSecsStr[6+1]; // used to output the 'microseconds' part of the presentation time
//  //sprintf(uSecsStr, "%06u", (unsigned)presentationTime.tv_usec);
//  //cout<< "Presentation time = " << (int)presentationTime.tv_sec << "." << uSecsStr;
//
...
//#endif
  ...
  {
	  isStartThread = true;
	  //mThread = new std::thread(detecConnectionLost, this);
	  //mThread = std::shared_ptr<std::thread>(new std::thread(detecConnectionLost, this));
	  //mThread = std::unique_ptr<std::thread>(new std::thread(detecConnectionLost, this));
  }
		  ...
		  unsigned long timestamp, void* user_data, unsigned long reserved1, unsigned long reserved2)
{
	...
	//  vf.height = pThis->mVideoHeight;
	// 	vf.height = vf.height >> 4;
	// 	vf.height = vf.height << 4;
	...
	//cout<<"Send encode data to "<<pThis->userParams
