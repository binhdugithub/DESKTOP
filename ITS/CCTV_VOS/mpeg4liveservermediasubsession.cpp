...
{
	std::cout << "Mpeg4LiveServerMediaSubSession::getAuxSDPLine -rtpSink="<<rtpSink<<" -sdpline="<<fAuxSDPLine << std::endl;
...
FramedSource* Mpeg4LiveServerMediaSubSession::createNewStreamSource(unsigned clientSessionID, unsigned& estBitRate)
{
    ...
    // for H264
    FramedSource *source = H264VideoStreamDiscreteFramer::createNew(envir(), mSource);
    return source;
...
{
	std::cout<<"createNewRTPSink; rtpGroupsock = " << rtpGroupsock <<std::endl;
	...
	//here is for H264
	return mySink = H264VideoRTPSink::createNew(envir(), rtpGroupsock, 96/*rtpPayloadTypeIfDynamic*/);
}
