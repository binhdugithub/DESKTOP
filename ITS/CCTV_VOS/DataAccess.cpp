...
{ // namespace DAL
	...
	DataAccess::DataAccess()
	{
		...
		mHost = std::string(Config::m_dbhost);
		mUser = std::string(Config::m_dbuser);
		mPassword = std::string(Config::m_dbpass);
		mDatabase = std::string(Config::m_dbname);
		mConn = NULL;


		std::cout << mUser << ":" << mPassword << "@" << mHost << ":" << mDatabase << std::endl;
	}
	...
	sql::Connection* DataAccess::GetConnection()
	{
		...
		if(mConn != NULL)
		{
			std::cout << "mConn con zin return luon" << std::endl;
			return mConn;
		}
