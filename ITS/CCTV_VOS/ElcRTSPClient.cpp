...
void ElcRTSPClient::continueAfterSETUP(RTSPClient* rtspClient, int resultCode, char* resultString)
{
  ...
  if (ourClient != NULL) {
	  ourClient->scs.sessionTimeoutParameter = rtspClient->sessionTimeoutParameter();
  }
...
void ElcRTSPClient::shutdownStream(RTSPClient* rtspClient, int resultCode)
{
	...
	env.taskScheduler().unscheduleDelayedTask(scs.sessionTimeoutBrokenServerTask);
	if (scs.session != NULL) {
...
void ElcRTSPClient::checkSessionTimeoutBrokenServer(void* clientData) {
	OurRTSPClient* rtspClient = (OurRTSPClient*)clientData;
	StreamClientState& scs = rtspClient->scs; // alias

	// Send an "GET_PARAMETER" request, starting with the second call
	if (scs.sessionTimeoutBrokenServerTask != NULL) {
		rtspClient->sendGetParameterCommand(*scs.session, NULL, NULL, NULL);
	}
	...
	scs.sessionTimeoutBrokenServerTask
		= rtspClient->envir().taskScheduler().scheduleDelayedTask(secondsUntilNextKeepAlive*1000000,
			(TaskFunc*)ElcRTSPClient::checkSessionTimeoutBrokenServer, clientData);
}
