...
SourceManager::SourceManager()
{
	...
	cout<<"All source in DB:" << camip.size() << " lines" << endl;
	for(size_t i = 0; i < camip.size(); ++i)
		cout<<"camip "<<camip[i]<<"  "<<inputurl[i]<<endl;

	mRTSPServer = new ElcRTSPServer(8554, NULL);
	if(!mRTSPServer->initServer())
	{
		std::cout << "Init rtsp server failed" << std::endl;
		LOG_ERROR("Init rtsp server failed");
		exit(0);
	}
...
int SourceManager::start(const std::string& url, string& output)
{
	...
	std::cout << __FILE__ << "_" << __FUNCTION__ << std::endl;
	LOG_INFO("SourceManager::start -url="<<url);
	int pos = findSource(url);
	if(pos < 0)
	{
		std::cout << "SourceManager::start  source not found" << std::endl;
	...
	if(client == NULL)
	{
		std::cout << "client or server is null" << std::endl;
	}

	if(client->isStart)
	{
		std::cout << "SourceManager::start  source has already started"<< std::endl;
	...
	{
		std::cout << "SourceManager::start cannot start rtsp client (ie. camera connection, cannot get sdp ..." << std::endl;
	...
	std::cout << "SourceManager::start start server" << std::endl;
	LOG_INFO("SourceManager::start start server");
	if(mRTSPServer->startStreaming(url, output) != voverlay_error::VOVERLAY_OK)
	{
		std::cout << "SourceManager::start cannot start streaming" << std::endl;
...
int SourceManager::stop(const std::string& url)
{
	...
	mRTSPServer->stopStreaming(camip[pos]);
	if(client->isStart)
		client->stop();
...
{
	LOG_INFO("SourceManager::startAllSource");
	if(inputurl.empty())
		return -1;
...
void SourceManager::createAllSource()
{
...
int SourceManager::createSource(const string& url)
{
	...
	if(!mRTSPServer->setSource(url, inputname[pos]))
	{
		std::cout << "setsource for " << url << " failed" << std::endl;
