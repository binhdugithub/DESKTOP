...
int main(int argc, char *argv[])
{
	...
	//soap
	std::cout << "Start SOAP Server" << std::endl;
	LOG_INFO("Start SOAP Server");
	videooverlayService *videoOverlaySvr = new videooverlayService();
	std::thread *m_hsoap = new std::thread(StartSoap, videoOverlaySvr);
...
void StartSoap(void* p_param)
{
	videooverlayService *vdovl = (videooverlayService*)p_param;

	while (1)
	{
		if(vdovl->run(Config::svrport))
		{
			std::cout << "Soap listen failed" << std::endl;
			vdovl->soap_stream_fault(std::cerr);
		}
		sleep(2);
	}

	std::cout << "End StartSoap" << std::endl;
}
