...
{ // namespace DAL
	...
	sql::ResultSet* DataAccess::QueryData(std::string sql)
	{
		try
		{
	...
	int DataAccess::QueryUpdate(std::string sql)
	{
	...
	int DataAccess::QuerySystem(std::string sql)
	{
		...
		try
		{
			...
			if(l_conn)
			{
				...
				{
					std::cout << sql << std::endl;
					int rowEffect = stmt->execute(sql);
					l_conn->commit();
